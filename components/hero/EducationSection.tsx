...
const EducationSection: React.FC<EducationSectionProps> = ({ eduData = [], sectionRef }) => {
  return (
    <SectionWrapper ref={sectionRef} id="education" title="Academic Background" terminalCommand="echo $education">
      {eduData && eduData.length > 0 ? (
        <motion.div variants={staggerContainer} className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {eduData.map((edu, index) => (
            <GlassCard key={edu.id || index} className="flex flex-col justify-between h-full group hover:border-neon-rose/40 transition-colors">
              <div>
                ...
                <p className="text-sm font-semibold text-neon-indigo mb-4 font-sora">
                  {edu.college}
                </p>
              </div>
